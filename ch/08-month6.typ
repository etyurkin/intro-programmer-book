...
Секреты: учебный `task/task` в compose — ок, это песочница. Токен от почты, пароль «как на проде», ключ AWS, который ты «на секунду» положил — не ок. Даже в истории git. Если уже запушил — ротируй, не делай вид, что «никто не видел». Видели боты.
...
#repl-note[
  `(ev '(+ 1 2))` → `3`. `(ev '(- 5 3))` → `2`. `(ev '(+ 1 (- 5 3)))` → `3`. Ты только что написал язык размером с напёрсток. Наперсток важнее, чем «я читал про компиляторы». На собесе можно улыбнуться: «для себя считал скобки». Кто-то улыбнётся в ответ. Этого достаточно.
]
