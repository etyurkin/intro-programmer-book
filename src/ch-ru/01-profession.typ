...
3. Написать.
4. Увидеть, что оно что-то делает не так или падает.
5. Починить.
6. Ещё раз. Ещё. Ещё.
...
Это скучнее, чем в кино. В кино хакер в чёрном капюшоне за три секунды взламывает Пентагон. На работе джун сорок минут выясняет, почему кнопка «сохранить» не сохраняет, и причина — пробел в конце строки. Потом он это чинит. Потом пишет тест, чтобы пробел больше не прилетал. Потом идёт пить чай. Это и есть профессия. Если такой день тебя бесит заранее — может, профессия не твоя. Если такой день кажется нормальным ремеслом — добро пожаловать на станцию.
...
- Spring Boot: контроллер принимает HTTP, сервис — правила, репозиторий — база, транзакция на сервисе.
...
Иногда день красивый: нашёл, почему прод глючит по понедельникам, и это была таймзона. Иногда день тупой: переименовать поле в пяти местах. Оба дня — работа. Романтизировать только красивые — путь выгореть к третьему месяцу, когда красивых мало.
...
- *Middle* — тебе доверяют кусок системы, а не одну кнопку. Ты уже говоришь «это плохая идея» и иногда тебя слушают. Зарплата растёт. Сообщений в Slack меньше не становится.
...
- *Architect* — решаешь, как устроена станция: какие отсеки есть, как стыкуются, что будет, если этот люк открыть. Это не управление людьми — управлять людьми можно и не хотеть. Стрелочки на схеме — следствие. Дело — решения, от которых зависит, развалится станция через полгода или нет. Иногда споришь, нужен ли Kafka *на этот раз* по делу.
...
На первой неделе надо, чтобы `Hello` печаталось. Карьера — это то, что вырастает из кучи вечеров, а не из слайда «траектория».
...
#os[
  Интерфейс IntelliJ можно оставить русским, если так спокойнее. Ошибки компилятора всё равно будут на английском. Привыкай читать ошибки, не переводить каждое слово в Яндексе: к третьей неделе `cannot find symbol` станет таким же бытовым, как «нет молока».
]

Не надо ждать «сначала выучу английский, потом Java». Нужен уровень «прочитал абзац документации и не умер». Он вырастает по дороге, если не избегать английских README как огня.
...
Ещё Lisp лечит страх ошибки. В REPL сломал — получил сообщение, починил, живёшь. Нет пятиминутной сборки, чтобы узнать, что забыл скобку. Этот рефлекс («сломал → читаю → чиню») потом спасает в Java, где сборка как раз пять минут, а ошибка — одна забытая скобка.

== Полгода — это плотно

Будни: два сорок. Плюс суббота. Если к двенадцатой неделе нет своего сервера с базой — не Кафку подкручивай, а доделай сервер. Интерес дешевле потерянного месяца. Пропустил неделю — не читай три главы за субботу, это не подвиг, это каша в голове. Возьми текущее занятие и свой проект.
...
Если работаешь на другой работе полный день — шесть месяцев могут растянуться в восемь месяцев. Нормально. Календарь в книге — план, не таймер бомбы. Станция «МОДУЛЬ» тоже строилась не по спринту из Jira, судя по изоленте.
...
Списать квест из отгадки в первую минуту — как посмотреть ответ в кроссворде и решить, что ты умный. Кажется, что понял. Не понял. Через месяц на собесе кроссворд будет без отгадки в конце книги.

Подсмотреть через двадцать минут неудач — нормально. Ты уже подумал. Мозг получил запрос. Отгадка тогда *ложится*, а не пролетает.

Код из этой книги — можно в свои учебные репозитории. Чужие книги и коммерческие репозитории — нельзя выдавать за своё. Станция и так на изоленте, давай без судебных исков и без вранья в резюме. Враньё на орбите кончается открытым шлюзом.
