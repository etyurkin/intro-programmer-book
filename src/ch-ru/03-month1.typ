...
Первый месяц — не «выучить программирование». Это месяц, когда компьютер из мебели превращается в существо, которое можно спросить и которое иногда отвечает. Иногда ругается. Иногда молчит, и это хуже.
...
SBCL ответит чем-то вроде `*ENERGY*`. Он пишет большими буквами, потому что внутри символы живут без учёта регистра: `*energy*` и `*ENERGY*` — одна бирка. Звёздочки — не синтаксис, а привычка лисперов кричать: «это глобальная, не наступи». Как табличка «мокро».
...
`GREEN`, `YELLOW`, `RED`. SBCL снова пишет капсом. Это те же символы.
...
#repl-note[
  `format t` пишет в терминал. `t` здесь — «стандартный вывод», не «да», хотя это тот же символ, жизнь сложная. `finish-output` — вытолкни буквы из трубы *сейчас*, не копи их до конца строки. Без этого на некоторых системах приглашение появится *после* того, как ты уже что-то ввёл. Очень смешно, один раз.
...
=== Java: типы, которые следят, чтобы ты не перепутал болт с гайкой
...
Фигурные скобки — загон для кода. Точка с запятой — «предложение кончилось». Без неё Java не начинает следующее предложение, она ругается.
...
`3.0` — чтобы деление было дробным. `(a + b + c) / 3` для целых отрежет хвост. Java считает: целые на целые — целое. Кладовщик здесь не ругается, он *молча* ворует дробь. Это хуже ошибки.
...
=== Если сломалось: Java ругается, и это нормально

Компилятор пишет много. Читай *первую* ошибку, не последнюю. Часто первая рожает остальные.

*Класс не как файл.* В файле `Energy.java` написал `public class Energia`. Пишет: class Energia is public, should be declared in a file named Energia.java. Переименуй либо файл, либо класс. Они одно существо.
...
#exercise("1.L3", "Lisp")[
  `lamp`: энергия 0..100. Верни `'green` / `'yellow` / `'red` теми же порогами, что `status` в тексте (80 и 40). Вызови на 100, 80, 79, 40, 0. Пять вызовов, пять ответов, в комментарии рядом. Если хоть один ответ не тот — пороги или порядок `cond`.
]
...
#exercise("1.J2", "Java")[
  Игра: машина загадывает 1..10 (`Math.random()`), ты один раз угадываешь. «верно» / «мало» / «много».
]
...
#warn[
  `(append '(a) '(b))` склеивает *копией*. `(cons '(a) '(b))` даёт `((A) B)` — список в голове, не склейка. Если ждал `(A B)`, а получил скобки внутри, ты взял не ту функцию. `append` сегодня можно один раз потрогать и не полюбить: для учёбы `cons` + `reverse` честнее, когда дойдёшь.
]
...
#repl-note[
  `List<String>` — «список строк». Уголковые скобки — какого зверя кладём внутрь. Это *джененерик*, слово для собеса. Положишь потом `rooms.add(3)` — кладовщик ругается: 3 не строка. `get(1)` — второй элемент, потому что ноль. `size()` — сколько сейчас, не «сколько влезет». Влезет много. Память кончится раньше, чем `ArrayList` скажет «нет».
...
В конце смены коридор на экране совпадает с коридором за спиной. Почти. В списке ещё есть `garden`, а за люком — ящики. В данных ещё старое прошлое. Ты пока не чинишь станцию. Ты чинишь список. Это уже работа.
...
#warn[
  Печать внутри `spend` — медвежья услуга. Захочешь посчитать сумму трат без печати — уже не получится, `format` прибит гвоздём. Раздели: считать / печатать. `render` возвращает строку, `print-...` пишет. Квест 3.L2 именно про это, не про красоту.
]

Сломай `clamp`: перепутай `lo` и `hi`. `(clamp 5 10 0)`. Лестница `cond` может вернуть чудо. Напиши два вызова в REPL, пока не увидишь, что ответ не тот. Потом верни.
...
1. Два `add`, печать id. Ожидай 1 и 2, не 0 и 1. Если нули — `nextId` начал с нуля, люди на станции считают с единицы, капитан будет орать.
...
#exercise("3.L2", "Lisp")[
  `render-room` возвращает строку. `print-rooms` только печатает. Раздели «что сказать» и «как напечатать в терминал».
]
...
`Scanner.nextInt()` при буквах бросает `InputMismatchException`. Поймай и попроси ещё раз. Или, как на занятии 1, читай строку:
