...
== Зачем учить языки, если есть ИИ

ИИ сделает. Сервис, тест, даже README, где ты «уверенно владеешь Kafka». Быстро. Иногда даже правильно.

Беда в вопросе. Модель отвечает на то, что ты спросил. «Напиши бэкенд» — получишь собор без плана эвакуации. «Почему 1 + 2 напечаталось как 12» — получишь нормальный ответ, *если* ты уже знаешь, что число и строка — разные вещи. Это не из чата. Это фундамент: переменная, ошибка, файл — не база, тест — не ритуал.

Без фундамента правильный вопрос не задать. Задашь «сделай чтобы работало» и будешь сидеть в чужом коде как в тёмном отсеке без фонарика. На собесе фонарик спросят у тебя. Модель в комнату не пустят.

#slow[
  ИИ ускоряет того, кто видит, *куда смотреть*. Того, кто не видит, он везёт в чужой репозиторий и оставляет там одного.
]

== Два языка, и это не путаница

*Lisp* здесь не для резюме. Резюме Lisp почти никому не нужен, и это прекрасно: можно делать глупости. Сорок минут. Прочитал кусок → набрал в REPL → *обязательно сломал и починил* → написал крошечную свою поделку. Если не сломал — ты не занимался, ты смотрел. Смотреть сериал про сварку и варить — разные профессии. Главное — не превращать Lisp в конспект лекции. Конспект про скобки — это уже не скобки.

*Java* — то, за что потом платят. Два часа. Из них минут двадцать посмотреть, как идея устроена, и полтора часа заставлять её работать *у тебя*. Ошибки, документация, Stack Overflow, глупый вопрос соседнему чату — это не провал. Это профессия, только без зарплаты. Зарплата появится, когда ты научишься не падать в обморок от сообщений об ошибках.
...
#slow[
  Lisp отвечает на вопрос «а как это *вообще* устроено?». Java отвечает на вопрос «а как мне за это *заплатят*?». Они не дерутся. Они живут в разное время дня, как чай и кофе. Смешивать в одной кружке не запрещено, но вкус будет странный.
]
...
Воскресенье можно гулять. Серьёзно. Мозг дописывает связи, пока ты идёшь за хлебом. Если вместо хлеба открыть четвёртую главу — на понедельник останется каша в голове и обида на скобки.

#rule[
  Узнал штуку — сразу в код. Не читай про хранилища Spring два часа. Через двадцать минут: «ок, теперь это должно появиться в *моём* проекте». Дальше пусть ругается ошибками. Так и задумано.
]
...
ИИ — как сосед по комнате, который это уже видел: «почему не компилируется», «что за иероглифы в ошибке». Не как джинн: «напиши мне сервис». На собесе джинна в кабинет не пустят, а тебя спросят, *зачем* тут это поле.
...
Пропустил вторник — в среду не читай вторник *и* среду. Читай вторник. Среда подождёт. Три главы за субботу — не подвиг, это каша в голове: скобки смешаются с аннотациями, и ты решишь, что «я тупой». Ты не тупой. Ты просто накормил мозг тремя ужинами сразу.
