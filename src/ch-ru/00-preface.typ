...
Беда в вопросе. Модель отвечает на то, что ты спросил. «Напиши бэкенд» — получишь собор без плана эвакуации. «Почему кнопка сохраняет пробел» — получишь нормальный ответ, *если* ты уже знаешь, что строка умеет врать по краям. Это не из чата. Это фундамент: переменная, ошибка, файл — не база, тест — не ритуал.
...
*Lisp* здесь не для резюме. Резюме Lisp почти никому не нужен, и это прекрасно: можно делать глупости. Сорок минут. Прочитал кусок → набрал в REPL → *обязательно сломал и починил* → написал крошечную свою дрянь. Если не сломал — ты не занимался, ты смотрел. Смотреть сериал про сварку и варить — разные профессии. Главное — не превращать Lisp в конспект лекции. Конспект про скобки — это уже не скобки.
