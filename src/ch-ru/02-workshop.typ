...
Пока нет верстака, эта книга — просто текст, который нельзя укусить. Сегодня ставим инструменты и пишем по крошечной программе на каждом языке. Если что-то не ставится — поздравляю, ты уже на работе: читай сообщение об ошибке *целиком*, не только первое слово.
...
Сломай специально. Убери точку с запятой после `println(...)`. `javac` напишет `';' expected` и номер строки. Это самое вежливое сообщение станции. Потом верни точку. Потом поставь класс `Hello` в файл `Bye.java` и посмотри, что скажет. Потом верни. Цель — не идеальный файл, а рефлекс: ошибку читают, не закрывают.
...
Если git пишет `Please tell me who you are` — ты пропустил `git config`. Сделай и коммит ещё раз.
...
#repl-note[
  Ошибка — не оценка. Оценка будет на собесе, и то не за количество ошибок, а за то, умеешь ли ты их читать. Сегодня тренировка: сломал, прочитал, починил, живой.
]

Частые гости недели 0:

- `command not found` — нет программы или нет PATH.
- `';' expected` — Java ждёт точку с запятой. Посмотри строку, которую назвали, и строку *выше*: иногда компилятор указывает на строку *рядом*.
