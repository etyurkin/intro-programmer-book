...
Стоп. Это уже хочется упростить, потому что вложенность путает. Проще так:
...
Если `walk` ругается «люка нет», ты либо не в той комнате, либо ребро одностороннее, либо `*here*` не то, что думаешь. `(look)` перед паникой. Панель врёт меньше, чем память.
...
SBCL ругается примерно: переменная `REACTOR` не связана. Или пытается *вызвать* функцию `reactor`. Зависит, что стоит на этом имени. В любом случае ты не пошёл.
...
#warn[
  Не добавляй «случайный критический удар» на первой вахте. Сначала правило должно быть повторяемым: 12 инея, blast по 3, посчитай на бумаге, сколько вспышек. Если бумага и REPL расходятся — глючит код. Потом уже `(random 3)`, если зудит.
]
