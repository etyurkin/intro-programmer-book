...
#slow[
  ...
  Сравни с Java `HashMap`: там `get` сразу значение или `null`. Здесь два шага. Зато видно устройство: карман — тоже список. Всё списки, мы говорили.
]
...
Тест — отдельная программа. Она вызывает твою и ругается, если ответ не тот. Это не «проверка глазами». Глаза врут, когда устали. Тест не устаёт, он только бесит.
...
#slow[
  ...
  Каждый тест — *новый* `TaskStore`. Не делить один стор на все методы класса через поле «для скорости». Тесты начнут зависеть от порядка, а порядок JUnit не обещал. Станция и так на изоленте, не надо ещё и плавающие тесты.
]

Напиши тест, *который падает*, потом почини код. Иначе ты тестируешь компилятор, а не себя.

Ритуал падающего теста:

1. В `complete` временно ничего не делай (пустой метод, `done` не трогай).
2. Тест: добавь, `complete(id)`, `assertTrue(task.isDone())`.
3. Запусти — тест падает. Прочитай сообщение. Оно должно говорить про `true` vs `false`, не про «не компилируется».
...
На GitHub: `git push -u origin HEAD` с ветки, потом Pull Request — или сразу merge локально и `push` `main`, если ты один на репозитории. Оба пути честные для учебника. Не честно: править `main` вперемешку с экспериментом «а если complete ничего не делает» и пушить падающие тесты как будто так и надо.
...
#exercise("5.J2", "Java")[
  Сломай `complete`, убедись что падает, верни. В README одна фраза: как гонять тесты на твоей машине (мак / WSL / `mvnw.cmd`).
]
...
Инспектор глянул. Тесты зелёные. `complete` снова глючит, если выключить одну строку — и тест это видит. На «МОДУЛЕ» это первый датчик, который срабатывает *до* взрыва. Можно привыкнуть.
...
#slow[
  `format nil` — не пишет в терминал, а *возвращает строку*. Как `render-room` против `print-rooms`. Сначала статусная строка: версия, код, фраза. Потом заголовки. Потом *пустая строка*. Потом тело.
...
Кавычки в `title` сломают жесть. Квест говорит: не экранируй, учебный жестяной JSON, не для боя. Потом Jackson в Spring сделает это за тебя, и ты ему даже спасибо не скажешь.
...
#slow[
  ...
  `@PostMapping` — письмо `POST`. `@RequestBody` — тело письма разверни в `TaskRequest`. `record` в Java 21 — короткий класс: поле `title`, конструктор, геттер `title()`. Не копируй десять строк ритуала, если хватает одной.
...
На Windows без WSL: `mvnw.cmd spring-boot:run`. В IDEA — зелёная стрелка на класс `...Application` с `@SpringBootApplication`. В логе жди что-то про `Tomcat started on port 8080`. Нет этой строки — не «запустилось». Читай ошибку *выше*. Часто: порт занят, Java не 21, опечатка в пакете.

Потом в *другом* окне:

#os[
  На маке и в WSL перенос длинной команды — обратный слэш `\`. В cmd.exe — `^`. В PowerShell проще одна длинная строка. `curl` в Windows 10+ уже есть (`curl.exe`). Если PowerShell подменяет `curl` своим `Invoke-WebRequest` — зови `curl.exe`. Если ругается странно — зайди в Ubuntu (WSL) и дергай оттуда, сервер на `localhost` общий, если Java тоже в WSL. Java на Windows, curl в WSL: `localhost` иногда тот, иногда нет. Держи клиента и сервер в одной вселенной.
]
...
#exercise("6.L2", "Lisp")[
  `json-task`: id и title → `{"id":1,"title":"..."}`. Кавычки в title не экранируй — учебный жестяной JSON, не для боя.
]
...
Вахта седьмая. Контроллер разжирел: список, номера, создание, поиск. Вахтёр у шлюза ещё и варит сталь. Капитан кричит не потому что жестокий — потому что когда Земля пришлёт вторую дверь (консоль, бот, «ещё один контроллер»), правила разъедутся. Один раз вынеси мозг внутрь.
...
#repl-note[
  `{}` — дырка, подставится аргумент. Не клеить `"id=" + id` как в коридоре: лог умеет лениво, и ты не разведёшь склейку строк зря, пока лог всё равно выключен. Пока горячего участка нет, но привычка дешёвая.
...
Если пункт врёт — это не «мелочь». Это дырка в шлюзе.
...
Чай. Коммит. Спать. Земля подождёт до утра, у неё и так полчаса пинга.
