...
Показать можно вот что: регистрация и вход, JWT, CRUD, пагинация. Миграции Flyway — схема в git, не «у меня в голове». На чтение списка стоит Redis на 30 секунд, на запись ключ сбрасываю; без сброса кэш глючит, я это специально ломал и писал в README.
...
HR не читает душу. HR читает, есть ли Java, есть ли ссылка, не кричит ли письмо «ГОТОВ К ВЫЗОВАМ!!!». Дальше письмо попадает к человеку, который *может* открыть GitHub. Ему нужна зацепка, не роман.
...
Тишина после десяти писем — тоже датчик. Проверь: есть ли ссылка, открывается ли compose, не ругается ли README на Windows. Иногда рынок просто медленный. Иногда иллюминатор грязный. Грязь ты чинишь. Медленный рынок — письмами дальше, не «пережду в стыде».
...
Если страшно — нормально. Страшно чинить реактор в первый месяц, страшно жать Send. Жать всё равно. Письмо, которое не ушло, не открывается. Станция, которую не включили, не пищит датчиками — и это не спокойствие, это тишина пустого коридора.
