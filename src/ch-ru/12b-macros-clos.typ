...
=== Когда макрос глючит: дважды посчитал
...
Это не «хуже Java» и не «лучше». Это другой центр: глагол важнее паспорта объекта. На станции удобно: ты всё время зовёшь `status`, а железяки разные.
...
#repl-note[
  ...
  Полезно, когда слотов станет пять и память уже глючит.
]
...
В Java цикл такой: написал → `javac` или Maven → собрал jar → остановил сервер → залил → запустил → молишься, что `main` тот. На телефоне ещё хуже: APK, установка, зелёная стрелка, активность родилась заново. Горячая замена в IDE иногда глючит. «Переписать метод у живого объекта» штатно язык не обещает.
