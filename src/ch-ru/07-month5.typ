...
Вакансии этого месяца звучат как список запчастей с чужой станции: Redis, очередь, Kafka, Docker, CI. Слова красивые. На собесе их произносят с умным лицом. Твоя задача — не выучить слова, а потрогать каждую штуку *на своём* монолите, пока она глючит, ругается и отказывается стартовать.

Если кэш глючит — ты это увидишь curl'ом. Если очередь не дренируется — увидишь в логе. Если в Docker база «не находится» — почти наверняка ты написал `localhost` внутри контейнера. Это не теория. Это вечер с чайником.
...
#lesson(17, [Кэш: быстрый, наглый, иногда глючит])
...
=== История первая: табличка глючит (stale cache)

Человек создал задачу «починить антенну». GET `/tasks` — видит. Потом переименовал в «починить антенну срочно». GET — всё ещё «починить антенну», без «срочно». Прошло десять секунд. Человек ругается, что сервер сломан. Сервер не сломан. Сломан *ты*: закэшировал список и забыл сбросить ключ на update.
...
#slow[
  ...
  `on: push` — любой толчок в репозиторий. `pull_request` — ещё и на PR, чтобы ты видел ошибку *до* слияния. Можно сузить ветки позже. Сейчас пусть падает всегда.
...
#os[
  Первый push с Windows: смотри, что `ci.yml` в LF, не в CRLF с сюрпризом. Git обычно сам. Если Actions ругается на странный символ в начале файла — BOM. Сохрани UTF-8 без BOM. На маке эта беда реже. Не значит «мак лучше». Значит «блокнот Windows иногда помогает слишком сильно».
]
...
#github[
  Коммит `week20: compose postgres redis app`. Тег, если хочется. README начинается с `docker compose up`, не с «открой IDEA». Иначе тег — пустышка.
]

#sunday[
  Сломай одну строку Dockerfile специально (`FROM` на несуществующий тег или опечатка в `ENTRYPOINT`). Прочитай ошибку вслух. Почини. Страх «докер слишком большой» проходит, когда ошибка стала конкретной, а не космической.
]
