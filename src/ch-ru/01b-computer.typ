...
- *Экран и клавиатура* — окошко в камбуз и рот, которым ты кричишь повару. Мышь — палец, который тыкает в картинки.
...
- *Файл не найден* — путь неверный: опечатка, не та папка, не тот диск.
...
Почему тогда IntelliJ и VS Code, если блокнот умеет текст? Потому что редактор для кода подсвечивает скобки, ругается на опечатки, прыгает к ошибке. Блокнот честный, но как чинить реактор отвёрткой для очков. Можно. Один раз. Потом возьми нормальный ключ.
...
Прервать программу, которая зависла и сыплет выводом:
...
Конец строки: Windows любит `\r\n` (два символа: верни каретку и новую строку). Linux и мак — `\n`. Git иногда пишет `LF will be replaced by CRLF`. Для учебного Java/Lisp почти никогда не смерть. Если скрипт «не запускается» и пишет странное `^M` — вот оно, наследие блокнота. В WSL: `file твой.sh` иногда подскажет. Потом научишься. Сегодня просто знай, что *текст не всегда одинаковый текст*.
...
#slow[
  ...
  Удалить файл — выкинуть банку. Запущенный процесс при этом *может ещё жить* (рецепт уже на стойке). Редко важно в первый месяц. Потом вдруг важно, когда «я стёр, а оно всё равно пишет».
]
...
`80`. Если в `.java` опечатка, `javac` ругается *сейчас*, и `.class` либо не родится, либо останется старый. Старый `.class` — отдельная подлость: ты починил исходник, забыл скомпилировать, запускаешь древнюю кашу. Поэтому зелёная стрелка в IDEA делает оба шага. В терминале помни про два.
...
Новичок видит ошибку и слышит: «я непригоден». Программист видит ошибку и слышит: «вот строка».
...
#slow[
  Ошибка — не оценка. Это датчик на станции. Датчик срабатывает, потому что воздух кончается, а не потому что ты плохой человек. Выключишь датчик (`catch` всего подряд, игнор ошибок в IDE) — тихо умрёшь в каюте. Читай датчик.
]

Что делать руками, всегда одно и то же:

1. Прочитать *целиком* первую ошибку. Не «первое слово ошибки». Строку.
...
`>` — «положи вывод в файл, затри старое». Если файл уже был — старое умрёт. Два знака `>>` — дописать в конец. Для учебной записки хватит редактора. `echo` — чтобы не уходить.
...
Каша из коридоров, разделённых двоеточием. Не учи наизусть. Узнай: *поиск идёт слева направо*. Два `java` в разных папках — победит тот, чей коридор раньше. Отсюда «в терминале старая Java, в IDEA новая».
...
- *stderr* — сюда пишет ошибки. Часто тоже на экран, но это *другая* труба. Поэтому можно сохранить вывод в файл, а ошибки всё равно увидеть.
...
Сравни в голове: «файл весит 12 байт» и «там 6 букв». Если UTF-8 и русский — сходится. Если кто-то открыл как Windows-1251, букв может стать «больше» или «крякозябры»: он режет два байта UTF-8 как две *разные* буквы 1251. Вот и крякозябры на кухне, без теории информации.
...
- `https` — *схема*: как разговариваться. `https` — HTTP плюс шифр. `http` — без шифра. Для учёбы хватит: «буквы до `://` — правило писем».
- `httpbin.org` — хост. Имя кухни. DNS переведёт в IP.
- `/get` — путь на *той* машине. Как папка, только у них. Не твой `/Users`.
- `?foo=1` — хвост запроса, query. Мелкие пары «имя=value». Иногда там живут сессии и утечки. Не клади сюда пароли.
