...
К концу шестнадцатой недели ты не «знаешь Spring Security». Ты можешь *вслух* объяснить, почему пароль не лежит текстом, почему `?userId=` в URL — дырка размером с шлюз, почему `HashMap` потерял ключ, и почему счётчик из двух потоков глючит. Вайтборд это любит. Кафка на вайтборде без этого — свисток без воздуха.
...
Тот же грех в красивом виде: `GET /users/7/tasks`. Выглядит как будто настоящий REST. Если 7 берётся из URL, а не сверяется с талоном — дырка та же, только в форме. Станция не обязана иметь «вложенные пользователи» в пути. `GET /tasks` и «кто ты» из SecurityContext — скучно и герметично.

=== Когда Security ругается, а ты ругаешься в ответ
...
=== Сессия EXPLAIN, с тысячами строк, иначе глючит
...
#slow[
  `EXPLAIN` без `ANALYZE` — план, как Postgres *собирается*. С `ANALYZE` — ещё и сколько заняло на самом деле, с живыми цифрами. Для README нужны оба прогона: до и после, на одном и том же объёме. Три строки в таблице — сравнение глючит как вежливый датчик: «всё в норме», а кислород уже свистит.
]
...
#exercise("14.J2", "Java")[
  README: `EXPLAIN` до и после. На трёх строках Postgres может индекс не взять — налей тысячи, иначе сравнение глючит как вежливый датчик.
]
...
*Grokking Algorithms* (там картинки) и 3–4 лёгкие задачи в неделю. Не медиум пачками. Медиум пачками — путь выгореть и возненавидеть скобки уже на Java.
...
#lesson(16, [Два запроса сразу, и почему счётчик глючит])
...
Lisp на этой неделе — чистота: без глобальных `defparameter`, только аргументы. Гонки в однопоточном SBCL нет, зато привычка таскать состояние аргументами потом помогает не плодить общее поле `int n` на весь сервер.

Можно начать крошечный свой язык: числа и `+`. Если число — верни. Если список с `+` — сложи `ev` хвоста. Это законно. Это даже желательно. Java от этого не отменяется.
